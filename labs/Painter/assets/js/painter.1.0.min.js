...
g.dispatchUpdateEvent()})}};this.applyFilterToSelectedLayer=function(a){var b=this.getSelectedLayer();b!=void 0&&(a=new Painter.LayerAction(Painter.LayerActionType.ApplyFilter,a),this.addLayerAction(b,a))};this.newLayer=function(){var a=this.getSelectedLayer(),g=new Painter.Layer(this);g.setName("\u65b0\u56fe\u5c42");var e=a!=void 0?h.indexOf(a)+1:void 0,j=new Painter.Recorder.Action(Painter.Recorder.Type.newLayer,e);Painter.HistoryManager.add("newLayer",function(){d(g);c(a);Painter.Recorder.removeAction(j)},
function(){b(g,e);c(g);Painter.Recorder.addAction(j)})};this.removeSelectedLayer=function(){var a=this.getSelectedLayer(),j=h.indexOf(a);if(a!=void 0){var f=e(a),i=new Painter.Recorder.Action(Painter.Recorder.Type.removeLayer,j);Painter.HistoryManager.add("removeLayer",function(){b(a,j);c(a);g.dispatchUpdateEvent();Painter.Recorder.removeAction(i)},function(){d(a);f!=void 0&&c(f);g.dispatchUpdateEvent();Painter.Recorder.addAction(i)})}};this.moveSelectedLayer=function(a){function b(){var c=h.indexOf(f),
...
g*=k;j=(j-e)*0.5;i=(i-g)*0.5;e=Math.round(e);g=Math.round(g);b.outerWidth(e);b.outerHeight(g);b.css("left",j);b.css("top",i+f);c.attr("width",e);c.attr("height",g);f=c.get(0).getContext("2d");f.clearRect(0,0,c.get(0).width,c.get(0).height);for(i=0;i<d.getLayers().length;){e=d.getLayers()[i];if(e.getVisible())j=e.getCanvas().get(0),f.save(),f.globalAlpha=e.getAlpha(),f.drawImage(j,0,0,j.width,j.height,0,0,c.get(0).width,c.get(0).height),f.restore();i++}}};Painter.HistoryPanel=function(){function a(){var a=Painter.HistoryManager.items.length,b=Painter.HistoryManager.idx;a==0?(d.addClass("disabled"),f.addClass("disabled"),e.addClass("disabled")):(e.removeClass("disabled"),b==a-1?(f.addClass("disabled"),d.removeClass("disabled")):(b==-1?d.addClass("disabled"):d.removeClass("disabled"),f.removeClass("disabled")));g.empty();a=Painter.HistoryManager.items.length;if(a==0)g.text("\u6ca1\u6709\u4efb\u4f55\u5386\u53f2\u8bb0\u5f55");else for(var b=[],c=Painter.HistoryManager.idx,
