...
Firefly.HistoryManager.go=function(b){for(b=this.items.indexOf(b);this.idx!=b;)b<this.idx?(this.items[this.idx].undo(),this.idx--):(this.idx++,this.items[this.idx].redo());$(document).trigger(Firefly.HistoryManager.UPDATE)};Firefly.HistoryItem=function(b,a,c){this.type=b;this.undo=a;this.redo=c};
Firefly.HistoryManager.addComponentAction=function(b){var a;this.add("new",function(){var b=Firefly.editor.getEditorCanvas();a=b.components[b.components.length-1];b.remove(a)},function(){a==void 0?Firefly.editor.runCmd(b):Firefly.editor.getEditorCanvas().add(a)})};
Firefly.HistoryManager.addSelectAction=function(b,a){Firefly.ArrayUtil.equals(b,a)?Firefly.editor.getEditorCanvas().scaleTool.setTargets(a):this.add(a.length>0?"select":"unselect",function(){Firefly.editor.getEditorCanvas().scaleTool.setTargets(b)},function(){Firefly.editor.getEditorCanvas().scaleTool.setTargets(a)})};
...
Firefly.HistoryManager.addMoveAction=function(b,a,c){this.add("move",function(){$.each(b,function(b){this.view.css("left",a[b].left);this.view.css("top",a[b].top);this.descriptor.update()});Firefly.editor.getEditorCanvas().scaleTool.update()},function(){$.each(b,function(a){this.view.css("left",c[a].left);this.view.css("top",c[a].top);this.descriptor.update()});Firefly.editor.getEditorCanvas().scaleTool.update()})};var lastTargets,firstPos;
...
b);$(document).unbind("mousemove",d);$(document).unbind("mouseup",f);c.view.hide()}b.preventDefault();$(document).bind("mousemove",d);$(document).bind("mouseup",f);var g=a.scaleTool.targets.concat();a.scaleTool.setTargets([]);var k={x:b.clientX,y:b.clientY};c.view.show();c.view.outerWidth(0);c.view.outerHeight(0);c.view.offset({left:k.x,top:k.y})})})();this.add=function(b){b.view.appendTo(this.contentLayer);this.components.push(b);b.view.mousedown(function(e){function d(b){b.preventDefault();l||(l=
!0);var c={x:b.clientX-h.client.x,y:b.clientY-h.client.y},b=h.scaleToolPos.x+c.x,c=h.scaleToolPos.y+c.y;if(a.enableAutoAlign)c=a.alignManager.update({x:b,y:c}),b=c.x,c=c.y;a.scaleTool.moveTo(b,c)}function f(){$(document).unbind("mousemove",d);$(document).unbind("mouseup",f);a.enableAutoAlign&&a.alignManager.clear();if(l){var b=a.scaleTool.targets.concat(),c=g();Firefly.HistoryManager.addMoveAction(b,m,c)}}function g(){var b=[];$.each(a.scaleTool.targets,function(){b.push(this.view.position())});return b}
e.preventDefault();e.stopImmediatePropagation();$(document).bind("mousemove",d);$(document).bind("mouseup",f);if(a.scaleTool.targets.indexOf(b)<0){var k=a.scaleTool.targets.concat();Firefly.HistoryManager.addSelectAction(k,[b])}var h={client:{x:e.clientX,y:e.clientY},scaleToolPos:{x:a.scaleTool.view.position().left,y:a.scaleTool.view.position().top}},m=g(),l=!1})};this.remove=function(a){this.components.splice(this.components.indexOf(a),1);a.view.remove()};this.toXmlNode=function(){var a=$("<div>");
a.width(this.descriptor.getWidth());a.height(this.descriptor.getHeight());a.css("background-color",this.descriptor.getColor());a.attr("ff-type","EditorCanvas");return a}};Firefly.Editor=function(b){function a(){var a=e.view.outerWidth()+1E3,f=e.view.outerHeight()+1E3,a=a>b.outerWidth()?a:b.outerWidth(),f=f>b.outerHeight()?f:b.outerHeight();c.outerWidth(a);c.outerHeight(f);a=(a-e.view.outerWidth())*0.5;f=(f-e.view.outerHeight())*0.5;e.view.css("left",a);e.view.css("top",f)}this.view=$("<div style='position:absolute;'></div>");this.view.outerWidth("100%");this.view.outerHeight("100%");this.view.css("overflow","auto");this.view.appendTo(b);var c=$("<div style='position:absolute;'></div>");
