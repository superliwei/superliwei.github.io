...
function(){f.hasClass("red")&&e.get(0).play()};this.stopMusic=function(){e.get(0).pause();e.get(0).currentTime=0};a.children().eq(2).text("GlobalEditor "+GlobalEditor.Settings.version);var q='<div style="color:#999999;text-align: center;">'+Base64.decode("Qmxhbms=")+"</div>";q+='<div style="color:#999999;text-align: center;">'+Base64.decode("aGVyb2JsYW5rQGdtYWlsLmNvbQ==")+"</div>";$(q).appendTo(a)}(a.eq(2),d.eq(2))],g=-1;e.each(function(a){$(this).click(function(){c.selectTabIdx(a)})});this.view.modal({onHide:function(){f[2].stopMusic()}});
...
name:"incrementallyLoadTextures"});this.add({type:"checkBox",label:"\u6295\u5c04\u9634\u5f71",name:"castShadows"});this.add({type:"checkBox",label:"\u63a5\u53d7\u9634\u5f71",name:"receiveShadows"});this.add({type:"checkBox",label:"\u64ad\u653e\u52a8\u753b",name:"runAnimations"});this.add({type:"dropdown",label:"height Reference",name:"heightReference",data:[{label:"CLAMP_TO_GROUND",value:Cesium.HeightReference.CLAMP_TO_GROUND},{label:"NONE",value:Cesium.HeightReference.NONE},{label:"RELATIVE_TO_GROUND",
value:Cesium.HeightReference.RELATIVE_TO_GROUND}]})};
